clear all;clc
addpath(genpath('/import/c4dm-datasets/ChickCallDataset/annotated/EUSIPCO22/')) % dataset
addpath(genpath('../jointscat_chickcall/')) %
run 'addpath_scatnet.m' % Scat code
tic

fid=fopen('file_names_callType.txt'); 
tline = fgetl(fid);
file_names = []; k=1;
while ischar(tline)
    file_names{k} = tline; 
...
cal_time = toc
save('chickcall_JTFS_feature.mat','fileFeatures', 'cal_time');